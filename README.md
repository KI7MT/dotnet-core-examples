...
are added, this document wull be updated as appropriate.
...
## Installation Using Makefile Files

Each application has its own `Makefile` while the commands to build, install,
uninstall and run are the same except where noted. For Windows, the file name is
`make.cmd`. For Linux, the file name is `Makefile` without extension. In both
cases, the invocation command is the same: `make <action or option>`.
...
# To Run Without Installing, type:
dotnet run
```
...
statements. All functions are contained within in the standard `(Program.cs)`
file. Typically, no other classes are involved apart from
...
|[Diamond](https://github.com/KI7MT/dotnet-core-examples/tree/master/Introductory/for-loops)|for-loop|Print a diamond using loops
...
interfaces, extend classes,  or more advanced
...
servers, web technologies, and application programming interfaces (API). As to their
classification, the examples fall into Advanced category. Setting up the Database
properly, and ensure the `Usernames and Passwords` match is key to a successful
launch.
